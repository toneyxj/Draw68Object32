 ...
 */
public class LocationImageLoader {
    /**
     * 图片缓存的核心类
     */
    ...
    private static LocationImageLoader mInstance;
     ...
     * @return
     */
    public static LocationImageLoader getInstance() {

        if (mInstance == null) {
            synchronized (LocationImageLoader.class) {
                if (mInstance == null) {
                    mInstance = new LocationImageLoader(10, Type.LIFO);
                }
            }
        }
        return mInstance;
    ...
    private LocationImageLoader(int threadCount, Type type) {
     ...
     */
    public void loadImage(final String path, final ImageView imageView, final Sucess sucess) {
        // set tag
        ...
        boolean is=true;
        if (bm != null) {
            is = Math.abs(bm.getHeight() - imageSize.height) < 200 && Math.abs(bm.getWidth() - imageSize.width) < 200;
     ...
     * @return
     */
    public static LocationImageLoader getInstance(int threadCount, Type type) {

        if (mInstance == null) {
            synchronized (LocationImageLoader.class) {
                if (mInstance == null) {
                    mInstance = new LocationImageLoader(threadCount, type);
                }
            }
        }
        return mInstance;
    ...
    public void ClearBitmapFromLruCache(String path){
        mLruCache.remove(path);
    }
...
//        }
    ...
    private Bitmap decodeSampledBitmapFromResource(String pathName,
        ...
        try {
            ...
            bitmap=createBitmap(reqWidth, reqHeight, Bitmap.Config.ARGB_8888);
        }
